...
public class InventoryTool {
    ...
    @Tool(description = """
            ...
            ВАЖНО: на каждый ресурс должна быть ОДНА проверка. Используй канонические имена:
              "cpu" — загрузка CPU, "memory" — RAM, "disk" — диск, "network" — сеть.
            Дедупликация двойная: по имени И по команде. Если команда совпадает с существующей проверкой — та удаляется.
            Команда должна выводить одно число. threshold — порог для открытия инцидента.
            minDurationMs — минимальная длительность нарушения (мс) перед открытием инцидента.
            Например 300000 (5 минут) — игнорировать кратковременные CPU-спайки. null — реагировать сразу.
            Чтобы изменить параметры — вызови с тем же name, старая запись удалится.
            """)
